...
from vit_pytorch.SS_TMNet import SS_TMNet
...
def get_model(name, **kwargs):
    ...
    if name == "nn":
        ...
        mlp_ratios = [3, 3, 3, 3]
        if dataset == "PaviaU":
            embed_dims = [208, 208, 512, 512]
        elif dataset == "IndianPines":
            embed_dims = [400, 400, 512, 512]
        elif dataset == "Houston":
            embed_dims = [288, 288, 512, 512]
        else:
            embed_dims = [208, 208, 512, 512]
        layers = [4, 3, 14, 3]
        ...
        segment_dim=segment_dim, mlp_ratios=mlp_ratios, mlp_fn=vit_pytorch.hit.ConvPermuteMLP, )
        lr = kwargs.setdefault("learning_rate", 0.0001)
        optimizer = optim.Adam(model.parameters(), lr=lr)
        criterion = nn.CrossEntropyLoss(weight=kwargs["weights"])
        kwargs.setdefault("epoch", 100)
        ...
        if dataset == "PaviaU":
            ...
            embed_dims = [248, 248, 512, 512]
        else:
            embed_dims = [168, 168, 512, 512]
        layers = [8, 5, 8, 5]
        model = SS_TMNet(layers, img_size=15, in_chans=n_bands, num_classes=n_classes, embed_dims=embed_dims, patch_size=3, transitions=transitions,
        segment_dim=segment_dim, mlp_ratios=mlp_ratios, mlp_fn=vit_pytorch.SS_TMNet.SSAM,)
        lr = kwargs.setdefault("learning_rate", 0.0001)
        optimizer = optim.Adam(model.parameters(), lr=lr)
        criterion = nn.CrossEntropyLoss(weight=kwargs["weights"])
        kwargs.setdefault("epoch", 100)
...
class HybridEtAl(nn.Module):
    ...
    def __init__(self, input_channels, n_classes, patch_size=15):
        ...
        self.conv4 = nn.Sequential(
            nn.Conv2d(2912, 64, (3, 3), padding=0, stride=(1, 1)),
            nn.ReLU()
        ) ## 6016 for IN, 576 for Grss ##2912 for PU
